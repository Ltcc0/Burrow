...
_Recently shipped: Developer ID signed & Apple-notarized macOS releases, Signed Sparkle feed + updater foundation, Stable Full Disk Access identity across updates, Bundled MIT engine — no separate `mo` install, Process inspector + CPU watchdog — see the [changelog](https://burrow.henryzh.dev/releases.html)._
<!-- ROADMAP:END -->
